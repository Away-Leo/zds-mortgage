			...
			<div class="p10">
			                        ...
			                        <td class="pct20">
			                            <dl class="form-item form-auto">
			                                <dd class="detail">
			                                        ...
			                                        <input class="zui-combobox zui-validatebox" validate-type="Require" id="credentialType" type="hidden" name="credentialType" 
			                    ...
			                    <tr>
			                        ...
			                        <td>
			                            <dl class="form-item form-auto">
			                                <dd class="detail">
			                                <label>
			                                    <input type="text" class="zui-date zui-validatebox strToDate" readonly validate-type="Require" id="birthday" onclick="WdatePicker({maxDate: '%y-%M-{%d-1}',realDateFmt:'yyyyMMdd',vel:'changeBirthday'})" onblur="setAge()">
		                            			<input type="hidden" id="changeBirthday" name="birthdayDate" />
			                                </label>
			                                </dd>
			                            </dl>
			                        </td>
				...
				<form id="marital_form" class="zui-form " method="post" enctype="multipart/form-data" style="display:none">
		                        ...
		                        <td>
		                            <dl class="form-item form-auto">
		                                <dd class="detail">
		                                        ...
		                                        <input class="zui-combobox zui-validatebox" validate-type="Require,Length[0-15]" id="maritalcredentialType" type="hidden" name="credentialType" value=""
						                          data-url="<z:res resource='public.simplecode.selector' isDefault='true'/>&jsoncallback=?&target=true&categoryCd=YWDM0025"
		            ...
		            <div class="page-box">
						...
						<div class="p10">
					        <div id="tb-contact" class="zui-datagrid" zdata-options='{"url":"<z:ukey key="com.cnfh.postLoanContact.findByClientId" context="admin"/>&jsoncallback=?","singleSelect":true,"pagination":true,"idField":"id","tableCls":"table-index","toolbar":"#contact-function"}'>
							    <table>
							        <thead>
							        <tr>
					...
					<div class="page-box">
						...
						<div class="p10">
					        <div id="tb-unit" class="zui-datagrid" zdata-options='{"url":"<z:ukey key="com.cnfh.postLoanWorkUnit.findByClientId" context="admin"/>&jsoncallback=?","singleSelect":true,"pagination":true,"idField":"id","tableCls":"table-index","toolbar":"#unit-function"}'>
							    <table>
							        <thead>
							        <tr>
						...
						<div class="p10">
					        <form id="unitedit_form" class="zui-form " method="post" enctype="multipart/form-data">
								...
								<dl class="form-item">
									...
									<dd class="detail">
				                           <input class="zui-combobox zui-validatebox" type="hidden"
                      						data-valuefield="fullcode" data-textfield="name" validate-type=""  name="industry" id="industryEdit" data-toggle="combobox">
									</dd>
								</dl>
								<dl class="form-item">
									<dt class="title"><b class="c-red mr5">*</b>单位性质：</dt>
		...
		seajs.use(['jquery','zd/jquery.zds.page.callback','zd/jquery.zds.address','zd/jquery.zds.form','zd/jquery.zds.message','zd/jquery.zds.dialog','zd/jquery.zds.combobox','zd/jquery.zds.table','zd/jquery.zds.seleter','zd/make-first-py'], 
	                        ...
	                        if (result.resultStatus == 0) {
                        		...
                        		if(data){
                					...
                					if(data.id ){
                						...
                						$("#formerName").val(data.formerName);
                						$("#credentialType").ZCombobox('setValue',data.credentialType);
                						$("#credentialNo").val(data.credentialNo);
                						$("#gender").ZCombobox('setValue',data.gender);
                						if(data.birthdayDate != '' && data.birthdayDate != null){
	                						$("#changeBirthday").val(data.birthdayDate);
	                						var birth = data.birthdayDate;
                						...
                						$("#maritalStatus").ZCombobox('setValue',data.maritalStatus);
                						...
                						$.ajax({
                		                    ...
                		                    data: {customerName:data.customerName , customerId:data.id,addressType:'t0930'},
                		                    dataType: 'json',
                		                    success: function (result) {
                		                        if (result.resultStatus == 0) {
                						...
                						$.ajax({
                		                    type: 'post',
                		                    url: '<z:ukey key="com.cnfh.postLoanAddress.findByCustomerNameAndCustomerId" context="admin"/>',
                		                    data: {customerName:data.customerName , customerId:data.id,addressType:'t0931'},
                		                    dataType: 'json',
                		                    success: function (result) {
                		                        if (result.resultStatus == 0) {
                		                        	var data = result.optional.postLoanAddressVo;
                						...
                						//有配偶给配偶赋值
                						if(data.maritalStatus == 'YWDM0011302' || data.maritalStatus == 'YWDM0011303' || data.maritalStatus == 'YWDM0011304' || data.maritalStatus == 'YWDM0011305'){
                						...
                						//联系方式列表加载
                						$("#tb-contact").ZTable("reload",{'customerId|E|S':data.id});
                						//工作单位列表加载
                						$("#tb-unit").ZTable("reload",{'customerId|E|S':data.id});
                					}
                				}
	                        }
	                    },
			...
			CALLBACK.setattr = function(index,value){
				...
				if(value == '身份证'){
					...
					$("#birthday").attr('onclick',"");
				}else{
					$("#credentialNo").attr('validate-type',"Require,Length[1-64]");
					$("#birthday").attr('onclick',"WdatePicker({maxDate: '%y-%M-{%d-1}',realDateFmt:'yyyyMMdd',vel:'changeBirthday'})");
				}
			}
		  ...
		  CALLBACK.copy = function(v,t){
			...
			CALLBACK.contactAdd = function(index, row){
		  		...
		  		if(id==null || id==""){
    				...
    				$("#contactTypeEdit").ZCombobox('setValue','');
    				$("#contactPhoneNumberEdit").val('');
	      ...
	      //联系方式编辑弹出框打开
			...
			CALLBACK.unitAdd = function(index, row){
		  		...
		  		if(id==null || id==""){
		  			...
		  			$("#unitIdEdit").val("");
                	$("#workUnitNameEdit").val("");
    				$("#companyNameEdit").val("");
    				$("#industryTypeEdit").ZCombobox('setValue',"");
    				$("#industryEdit").ZCombobox('setValue',"");
    				$("#workUnitNatureEdit").ZCombobox('setValue',"");
    				$("#workYearsEdit").val("");
    				$("#positionEdit").ZCombobox('setValue',"");
    				$("#unitPhoneNumberEdit").val("");
    				$("#workUnitAddressEdit").val("");
                  	//工作单位新增地址
