...
<div class="page-box">
	<div class="p10">
		<form id="credit_entrust_form" class="zui-form " method="post"
			...
			<input type="hidden" id="id" name="id" value="${creditEntrustDebitVo.id }"/>
			...
			<div class="page-box">
				...
				<dl class="form-item block">
	                ...
	                <dd class="detail">
		                <label>
		                	<textarea class="zui-area zui-validatebox" id="remark" name="remark" validate-type="Length[0-200]" placeholder="最多可以输入200个字符">${creditEntrustDebitVo.remark }</textarea>
		                </label>
	                </dd>
                </dl>	
			</div>
			<div class="page-box">
				...
				<div class="p10">
					<div id="log_datagrid" class="zui-datagrid" zdata-options='{"url":"<z:ukey key='com.zdsoft.finance.capital.getCreditLogs' context='admin'/>&jsoncallback=?&businessId|E|S=${creditEntrustDebitVo.id }","singleSelect":false,"pagination":true,"idField":"id","tableCls":"table-index"}'>
						<table>
							<thead>
			        			<tr>
...
<div id="chooseMember"></div>
		...
		seajs.use(['jquery','zd/jquery.zds.page.callback','zd/jquery.zds.form','zd/jquery.zds.message','zd/jquery.zds.dialog','zd/jquery.zds.combobox','zd/jquery.zds.table','zd/jquery.zds.seleter'], function($, CALLBACK) {
			...
			// 二级菜单获取费用项
			CALLBACK.getFeeItemT = function(index,rowData){
			    var debitType = $('#debitType').ZCombobox('getValue');
				if(index){
					$.ajax({
                        type: 'post',
                        url: '<z:ukey key="com.zdsoft.finance.capital.getFeeItems" context="admin"/>&jsoncallback=?&attribution='+debitType+','+index,
                        data: {},
                        dataType: 'json',
                        success: function (data) {
                            if (data.resultStatus == 0) {
			...
			$('#saveBtn').click(function(){
				...
				if(isValidate){
					...
					$.ajax({
	                    type: 'post',
	                    url: '<z:ukey key="com.zdsoft.finance.capital.saveEntrustDebit" context="admin"/>',
	                    data: param,
	                    dataType: 'json',
	                    success: function (data) {
	                        if (data.resultStatus == 0) {
