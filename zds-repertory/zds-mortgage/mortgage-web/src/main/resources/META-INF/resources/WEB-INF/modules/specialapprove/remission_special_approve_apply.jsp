			...
			<div class="p5">
				<table class="table-detail">
					...
					<tr>
						...
						<td class="pct20">
							${remissionBasicVo.principalBalance } 元
						</td>
...
<script type="text/javascript">
	seajs.use(['jquery', 'zd/jquery.zds.page.callback', 'zd/jquery.zds.table', 'zd/jquery.zds.form','zd/jquery.zds.dialog','zd/jquery.zds.combobox'],
		   function ($, callback) {
		...
		callback.getItemAmount = function(v,t){
				...
				success:function(datas){
					if(datas.resultStatus == 0){
						if(datas.optional){
							...
							$.ZMessage.error("错误",datas.msg,function(){});
						}
					}
				}
			})
	        ...
	        buttons: [{
                 ...
                 handler: function () {
                	 var rowDatas = $("#remissionDetail").ZTable("getRows");
	               	  var status = true;
	               	  $(rowDatas).each(function(index,row){
		 ...
		 $('#remissionDetail').ZTable({
	         ...
	         columns: [[
	             ...
	             {field: 'id', title: '操作', align: 'center', width: '15%',formatter:function(){
	         ...
	         onDelete: function (rowsIndex, rowsData) {
	             /* $.ZMessage.question("提示","确定要删除当前记录吗？",function(){
	            	 return true
	             }) */
	             return true;
	         }
		...
		function saveDate(isSubmit){
			//验证
			var isValidateForm = $.ZUI.validateForm($('#us'));
			if(!isValidateForm){
				return false;
			}
			...
			if(remissionDetail.length == 0 && isSubmit){
				$.ZMessage.error("提示", "减免明细不能为空", function(){});		
				return;
			}
			if(remissionDetail == undefined){
				return false;
			}
		...
		**/
		...
		'&linkCode=${fileStoreVo.linkCode }&caseApplyId=${fileStoreVo.caseApplyId }&businessId=${fileStoreVo.businessId }');
	});
</script>
</html>