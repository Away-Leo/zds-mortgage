...
<div class="page-box">
	<div class="p10">
		<form id="credit_entrust_form" class="zui-form " method="post"
            ...
            <div class="form-btn">
               	<button id="saveBtn" type="button" class="btn-blue">到账确认</button>
            </div>
		</form>
	</div>
</div>
		...
		seajs.use(['jquery','zd/jquery.zds.page.callback','zd/tools','zd/jquery.zds.form','zd/jquery.zds.message','zd/jquery.zds.dialog','zd/jquery.zds.combobox','zd/jquery.zds.table','zd/jquery.zds.seleter'], function($, CALLBACK,ZTOOL) {
			...
			$('#saveBtn').click(function(){
				...
				if(isValidate){
					...
					$.ajax({
	                    type: 'post',
	                    url: '<z:ukey key="com.zdsoft.finance.capital.confirmSpareCapital" context="admin"/>',
	                    data: param,
	                    dataType: 'json',
			...
			$(function(){
				$("#useDateLocal").val(ZTOOL.strToDate($("#useDateLocal").val()));
				$("#actualArrivalDateLocal").val(ZTOOL.strToDate($("#actualArrivalDateLocal").val()));
			});
		});
	</script>
</body>
