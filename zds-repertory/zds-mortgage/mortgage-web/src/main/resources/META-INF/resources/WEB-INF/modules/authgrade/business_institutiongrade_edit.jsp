            ...
            <div class="p10">
                <form action="javascript:void(0);" id="institutionSearchFrom"
                        ...
                        <dd class="detail">
                            <label>
                                <input class="zui-input zui-disabled" type="text"
                                       <c:if test="${isAuth ==null}">
                                           value="${orgAuthConn.originalGradeName}"
                                       </c:if>
                                       disabled="disabled"/>
                                <input type="hidden" id="originalGradeCode" name="originalGradeCode" <c:if test="${isAuth ==null}"> value="${orgAuthConn.originalGradeCode}"</c:if> />
                                <input type="hidden" id="originalGradeName" name="originalGradeName" <c:if test="${isAuth ==null}"> value="${orgAuthConn.originalGradeName}" </c:if> />
                            </label>
                        </dd>
                    </dl>
                    ...
                    <dl class="form-item">
                        ...
                        <dd class="detail">
                            <input class="zui-combobox zui-validatebox" type="hidden" id="finalGradeCode" name="authGradeCode" value="${orgAuthConn.authGradeCode}"
                            ...
                            <input type="hidden" id="finalGradeName" name="authGradeName" value="${orgAuthConn.authGradeName}" />
                        </dd>
                    </dl>
                    <dl class="form-item">
            ...
            <div class="p10">
                <div id="institutiontable">
                    <div id="zds-institution-table"
                         ...
                         zdata-options='{
                     "url":"<z:ukey key="com.zdsoft.finance.findHistoryData" context="admin"/>&jsoncallback=?&institutionCode=${institutionCode}",
                     "singleSelect":true,
                     "pagination":true,
                     "idField":"id",
    ...
    seajs.use(['jquery', 'zd/jquery.zds.page.callback','zd/jquery.zds.validate', 'zd/jquery.zds.combobox', 'zd/jquery.zds.checkbox', 'zd/jquery.zds.loading', 'zd/switch',
            ...
            function ($, CALLBACK, Loading, Switch, DropDown, Filter, Check, Zdialog, ZUI_MESSAGE_CLIENT) {
                ...
                $("#btn-save").click(function(){
                    ...
                    var url='<z:ukey key="com.zdsoft.finance.saveOrUpdateInstitution" context="admin" />';
                    $.ajax({
                        url:url,
                        data:param,
                        type:"post",
