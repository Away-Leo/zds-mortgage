...
<!-- 编辑附件对话框 -->
<div id="uploadAtta_customer_credit" style="display: none">
	<div class="p10">
		<form id="customer_credit_AttaForm" class="zui-form " method="post" enctype="multipart/form-data">
			<input type="hidden" id="creditId" name="businessId" value=""/>
			<div class="page-box">
				<dl class="form-item span2">
					<dt class="title"><b class="c-red mr5">*</b>文件类型：</dt>
					<dd class="detail">
	                      <input class="zui-combotree zui-validatebox" 
	                      		id="creditFileType" name="materiaCode" type="hidden"
					...
					<dd class="detail">
						<label> 
							...
							<span id="attachmentUploadStatus" class="f12">
								<span id="attrRemark">注：未上传附件</span>
							</span>
						</label>
					</dd>
				</dl>
...
seajs.use(['jquery','zd/jquery.zds.page.callback','zd/jquery.zds.form','zd/jquery.zds.message','zd/jquery.zds.dialog', 'zd/jquery.zds.seleter'], function($, CALLBACK) {
	// 附件上传对话框定义
	$("#uploadAtta_customer_credit").Zdialog({
		width: 700, 
		height: 350, 
		closed: true, 
		title:'附件上传',
		buttons: [{
			...
			handler: function () {
				...
				var validate = $.ZUI.validateForm($('#customer_credit_AttaForm'));
		    	...
		    	if (creditId == "") {
		    		$.ZMessage.error("错误", "未选择征信客户!", function () {});
		            return false;
		    	}
		    	...
		    	if (attachmentId == "") {
               	...
               	var datas = $("#customer_credit_AttaForm").serialize();
               	$.ajax({
                   type: 'post',
                   url: '<z:ukey key="com.zdsoft.finance.customer.credit.saveCustomerCreditAtta" context="admin"/>&caseApplyId=${caseApplyId}',
                   data: datas,
                   dataType: 'json',
                   success: function (data) {
                       if (data.resultStatus == 0) {
	                       	$.ZMessage.success("成功", "添加附件成功！", function () {
	                       		...
	                       		doSearch();
	                       		//
	                       		$("#uploadAtta_customer_credit").Zdialog('close');
	                        });	
                       }else{
...
window.uploadFileCallback=function (file, data, response){
	 ...
	 */
	showFileContent(attachements,"attachmentUploadStatus");
