...
<script type="text/javascript">
	seajs.use(['jquery','zd/jquery.zds.page.callback','zd/jquery.zds.message','zd/jquery.zds.dialog','zd/jquery.zds.table','zd/jquery.zds.form'], 
		function($, CALLBACK) {
		...
		$('#contactWayInfoList').ZTable({
	       url : "<z:ukey key='com.zdsoft.finance.beforeContact.listContactByCustomerId' context='admin'/>&jsoncallback=?&customerId=${persCustomerVo.id }",
	       singleSelect : true,
	       isRowNum : false,
	       pagination : false,
	       currentPage : 1,
	       ...
	       columns:[[
				...
				{field : 'id',title : '操作', align : 'center',formatter:function(r,v){
					return '<a href="javaScript:void(0)" onclick="editContactHandle" class="btn-blue" >编辑</a>&nbsp;&nbsp;<a href="javaScript:void(0)" onclick="deleteContactHandle" class="btn-blue">删除</a>';
				}}
		] ],
		...
		CALLBACK.deleteContactHandle=function(index, rowData){
			$.ZMessage.question("确认", "确认要删除这条数据吗？", function () {
				//模拟点击选中事件
				$($('#contactWayInfoList .datagrid-body').find("tr")[index]).trigger("click");
