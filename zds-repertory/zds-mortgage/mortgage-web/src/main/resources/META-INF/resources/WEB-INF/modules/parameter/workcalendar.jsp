...
<body>
	...
	<div style="margin:80px auto;width:500px;">
		<dl > 
			<dt style="border:1px solid #000000;width:20px;height:20px;background-color:#ff8040;">
			</dt>
			<dd>
				休息日
			</dd>
		</dl>
		<dl style="margin;left:40px;margin-top:10px;">
			<dt style="border:1px solid #000000;width:20px;height:20px;background-color:#ffffff;">
			</dt>
			<dd>
				工作日
			</dd>
		</dl>
		<div id="cc" style="margin-top:40px;">
		</div>
		<input id="setDay" style="margin:20px 180px;width:100px;height:30px;"  class="btn-blue" type="button" onclick="calendar.updateWorkDay()" value="设置为休息日">
	</div>
</body>
<script>
	$('#cc').calendar({
	    ...
	    onSelect : function(){
			...
			if(currentDay.css("background-color")=="rgb(255, 128, 64)"){
				$("#setDay").val("设置为工作日");
			}
			else{
				$("#setDay").val("设置为休息日");
				...
				for(var i=0;i<allDays.length;i++){
					if(days[j]){
						if(calendar.convertFormat($(allDays[i]).attr("abbr"))==days[j][0]){
							if(days[j][1]==0){
								$(allDays[i]).css("background-color","#ff8040");
							}
							else{
