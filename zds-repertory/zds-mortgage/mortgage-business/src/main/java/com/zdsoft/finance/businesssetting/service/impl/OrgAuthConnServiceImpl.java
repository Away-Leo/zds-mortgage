...
import com.zdsoft.finance.businesssetting.repository.OrgAuthConnRepository;
...
/**
 * 版权所有：重庆正大华日软件有限公司
 * @title OrgAuthConnServiceImpl.java
 * @className OrgAuthConnServiceImpl
 * @description 机构和授权等级设定service实现类
 * @author LiaoGuoWei
...
public class OrgAuthConnServiceImpl extends BaseServiceImpl<OrgAuthConn,OrgAuthConnRepository> implements OrgAuthConnService {
    ...
    public OrgAuthConn saveOrgAuthConn(OrgAuthConn orgAuthConn) throws Exception {
        if(ObjectHelper.isNotEmpty(orgAuthConn)&&ObjectHelper.isNotEmpty(orgAuthConn.getOrgIntermediateId())){
            ...
            if(ObjectHelper.isNotEmpty(sourceAuthGrade)&&sourceAuthGrade.size()>0){
                ...
                //删除原来已经存在的评级
                this.deleteByOrgCd(orgAuthConn.getOrgIntermediateId());
    ...
    public void deleteByOrgCd(String orgCd) throws Exception {
        if(ObjectHelper.isNotEmpty(orgCd)){
            this.orgAuthConnRepository.deleteByOrg(orgCd);
        }else{
            throw new BusinessException("10010004","未传入相关数据！删除原关系未传入相关机构编号");
        }
    }
}
