...
public class CreditEntrustPlanConfigRepositoryImpl {
	...
	public Page<CreditEntrustPlanConfig> findByPage(CreditEntrustPlanConfig creditEntrustPlanConfig,Pageable pageable){
		...
		if(ObjectHelper.isNotEmpty(creditEntrustPlanConfig.getProduct()) && ObjectHelper.isNotEmpty(creditEntrustPlanConfig.getProduct().getId())){
	...
	public List<CreditEntrustPlanConfig> findByProductIdAndCapitalistId(String productId, String capitalistId) {
		// hql
		StringBuffer hql=new StringBuffer("from CreditEntrustPlanConfig c where c.isDeleted=:isDeleted and c.product.id=:productId and c.capitalistId=:capitalistId ");
