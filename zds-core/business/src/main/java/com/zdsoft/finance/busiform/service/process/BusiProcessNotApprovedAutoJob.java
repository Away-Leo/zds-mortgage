 ...
 * 
 * 版权所有：重庆正大华日软件有限公司
 * @Title: BusiProcessNotApprovedAutoJob.java 
 * @ClassName: BusiProcessNotApprovedAutoJob 
 * @Description: 流程业务审批不通过
 * @author dengyy 
 * @version V1.0 
 * @date 2017年3月2日 上午11:10:05
 *
 */
@Service
@AutoJob(label = "业务审批不通过", resource = "com.zdsoft.finance.busiform.service.process.notApproved.autoJob")
public class BusiProcessNotApprovedAutoJob implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		if (ObjectHelper.isNotEmpty(busiObj)) {
			...
			if (ObjectHelper.isNotEmpty(entity)) {
				...
				entity.setFormStatus(BusiFormStatus.NOTAPPROVALED.value);
