define(function (require, exports, module) {
	...
	//构造函数
	function ZDSValidatebox() {
		this.name = "ZDSValidatebox";
		this.version = '1.0.0';
		this.text = "";
	}

	//初始化对象
	$.ZDSValidatebox = new ZDSValidatebox();
	...
	var methods = {
		...
		bindBox: function () {
			...
			if (that.is("textarea") && validateType && validateType.indexOf('Length') != -1) {
				...
				that.blur(function () {
					if (that.hasClass('zui-date')) {
						var selfVal = $(this).val();
						//清除验证失败提示信息
						if (selfVal || selfVal.length > 0) {
							$.ZUI.cleanValidateText($(this).parent().parent());
						}
					}
				});
			}
		...
		validateAll: function (obj) {
			var checked = true;
			$(obj).find('[class*="zui-validatebox"]').each(function () {
				if (!checkObj(this)) {
					checked = false;
				}
			});
	...
	//定义插件
	$.fn.ZDSValidatebox = function (options, param) {
		...
		options = options || {};
		var opts = $.extend({}, $.fn.ZDSValidatebox.defaults, options);
		var validateType;
		var validataMsg;
		if (null == opts.type) {
	...
	function checkObj(obj) {
		...
		if (typeof validateType == 'undefined' || '' == validateType || null == validateType) {
			...
			if (this_class.indexOf("combobox") > 0) {
				inputValue = $(obj).ZCombobox("getValue");
			} else if (this_class.indexOf("combotree") > 0) {
				inputValue = $(obj).ZComboTree("getValue");
			}
	...
	function showErrorMsg(obj, caseOption, str) {
		var this_class = $(obj).attr('class');
		var appendHtml = '<span class="form-error zd-validation" pop="reqFalse"><i class="icon-merror"></i><span class="form-msg">';
		if (!str) {
		...
		if (this_class.indexOf("combobox") > 0 || this_class.indexOf("combotree") > 0) {
			$(obj).parent().find('.zui-mselect').addClass('error');
			//obj的父亲是dd.detail
			$(obj).parent().find('.zd-validation').remove();
			$(obj).parent().append(appendHtml);
		}else if(this_class.indexOf("checkbox") > 0){
			$(obj).parent().append('<span class="form-error zd-validation" pop="reqFalse"><i class="icon-merror"></i></span>');
		}else {
			$(obj).addClass('error');
			//obj的父亲是lable，爷爷是dd.detail
			$(obj).parent().parent().find('.zd-validation').remove();
	...
	function showSuccessMsg(obj) {
		var this_class = $(obj).attr('class');
		//var appendHtml='<span class="zd-validation form-success">验证成功</span>';
		if (this_class.indexOf("combobox") > 0 || this_class.indexOf("combotree") > 0) {
			$(obj).parent().find('.zui-mselect').removeClass('error');
			$(obj).parent().find('.zd-validation').remove();
			//$(obj).parent().append(appendHtml);
		} else {
