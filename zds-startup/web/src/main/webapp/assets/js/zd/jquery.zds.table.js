define(function (require, exports, module) {
	...
	// 插件的定义
	$.fn.ZTable = function (options, param) {
		//如果为string，表示是方法调用
		...
		options = options || {};
		var opts = $.extend({}, $.fn.ZTable.defaults, options);
		...
		return this.each(function () {
			...
			//构建Table标题栏上面的按钮区域
			var div = $(this).append('<div class="operate datagrid-toolbar"></div>');
			var tb = div.children('.datagrid-toolbar');
			if (opts.toolbar) {
				$.each(opts.toolbar, function (n, toolbar) {
					tb.ZButton($.extend({}, toolbar, {groupDiv: tb}));
				});
			} else {
				tb.css("display", "none");
			}
			...
			if (opts.columns) {
				var table_start = '<div class="zd-table table-scroll"><table class="' + opts.tableCls + '">',
					table_end = '</table></div>',
				...
				header_html += '<th id="datagrid-header-check" style="width:30px;' + display + '">' +
					'<input name="ck" type="checkbox" title="全选"></th>';

				//行号
				if (opts.isRowNum) {
					header_html += '<th id="datagrid-header-rownumber" style="width: 36px;;">行号</th>';
				}
				...
				//构建table最基本的html结构
				if ($(this).find(".zd-table").length > 0) {
					$(this).find(".zd-table").html(table_start
						+ table_header
						+ header_html
						+ table_header_end
						+ talbe_body
				...
				//设置默认的分页参数和值
				opts.queryParams.rows = opts.rows;
				if (opts.thisQueryPage) {
					opts.queryParams.page = opts.thisQueryPage;
	...
	$.fn.ZTable.methods = {
		 ...
		 * */
		getRows: function (jq) {
			return $(jq).data("ztable").options.data.rows;
		},
		...
		selectRow: function (jq, param) {
			...
			for (var i = 0; i < options.data.rows.length; i++) {
				//如果值相等，表示就是当前行
				if (options.data.rows[i][options.idField] == param) {
					index = i;
				}
			}
		...
		addRow: function (jq) {
			...
			if (options.pagination) {
				$.ZMessage.warning("警告", "分页的表格不能直接添加行");
				return;
			}
			//复选框
			if (options.singleSelect) {
			...
			rowsHtml += '<td  field="datagrid-td-check" value="' + rowId + '"  style="'
			...
			$.each(options.columns[0], function (c, column) {
				...
				switch (type) {
					case 'text':
						tdHtml += '<input type="text" class="zui-input"  style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>';
						break;
					case 'date':
						tdHtml += '<input type="text" class="zui-date" onclick="WdatePicker()" readonly  style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>';
						break;
					case 'textarea':
						width = options.data.type[1].areaWidth;
						height = options.data.type[1].areaHeight;
						tdHtml += '<textarea class="zui-area" style="width:' + width + 'px;height:' + height + 'px"></textarea>';
						break;
					default:
						tdHtml += '<input type="text" class="zui-input"  style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>';
				}
			...
			$saveColumn.append('<a class="datagrid-btn btn-search-blue ml10">保存</a>');
			...
			$saveColumn.find('.datagrid-btn').on('click', function (e) {
				...
				var $row = $(this).parent().parent(),//当前行
					...
					rowsData = options.data.rows,//所有行数据
		...
		deleteRow: function (jq) {
			...
			//没有选中任何行
			if (len === 0) {
				$.ZMessage.warning("警告", "请选择要删除的表格行");
				return;
			}
			...
			for (var i = 0; i < len; i++) {
				//存储删除的行数据
				rowsData.push(options.data.rows[selectRows[i]]);
				//删除行数据
				options.data.rows.splice(selectRows[i] - i, 1);
			}
		...
		editRow: function (jq) {
			//定义变量
			var options = $(jq).data("ztable").options,
				rowsDate = options.data.rows,//当前所有行数据
			...
			//没有选中编辑行
			if (len <= 0) {
				$.ZMessage.warning("警告", "请选择要编辑的表格行！");
				return false;
			}
			...
			for (var i = 0; i < len; i++) {
				...
				trObj.find('td').removeClass('selected');
				...
				$.each(options.columns[0], function (c, column) {
					...
					switch (type) {
						case 'text':
							tdObj.append('<input type="text" class="zui-input" value="' + tdText
								+ '" style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>');
							break;
						case 'date':
							tdObj.append('<input type="text" class="zui-date" onclick="WdatePicker()" readonly  value="' + tdText + '" style="width:' + width + 'px;height:' + height + 'px;line-height:' + height + 'px;"/>');
							break;
						case 'textarea':
							width = options.data.type[1].areaWidth;
							height = options.data.type[1].areaHeight;
							tdObj.append('<textarea class="zui-area" style="width:' + width + 'px;height:' + height + ' px" >' + tdText + '</textarea>');
							break;
					}
				});
				trObj.find('td').last().append('<a class="datagrid-btn btn-search-blue ml10" >保存</a>');
			...
			$table.find("tr").unbind("click").bind("click", function () {
				...
				$(this).find(':checkbox[name=ck]').each(function () {
					...
					if (!nowIndex) {
						...
						options.onSelect.call(this, myIndex, options.data.rows[myIndex]);
					} else { // 第二次点击
						...
						if (_index == inIndex) {
							...
							nowIndex = null;
							options.selectRows.length = 0;
							options.onUnselect.call(this, inIndex, options.data.rows[inIndex]);
						} else {
							// 将以前选中的去掉，选中现在行
							nowIndex.removeAttr('checked');
							$(this).attr('checked', 'checked');
			...
			$table.find('.datagrid-body').find("tr").unbind("click").bind("click", function () {
				...
				$(this).find(':input').each(function () {
					...
					if ($(this).attr('trSelect')) {
						...
						options.onUnselect.call(this, _index, options.data.rows[_index]);
					} else {
						if ($(this).attr('checked')) {
							...
							options.onSelect.call(this, _index, options.data.rows[_index]);
						} else {
							$(this).attr('checked', 'checked');
							$(this).attr('trSelect', true);
							options.selectRows.push(_index);
	...
	function loadData(obj, opts) {
		...
		var options = $(obj).data("ztable").options;
		//设置默认的分页参数和值
		options.queryParams.rows = options.rows;
		if (opts.thisQueryPage) {
			options.queryParams.page = opts.thisQueryPage;
			options.currentPage = opts.thisQueryPage;
			opts.thisQueryPage = 0;
		} else {
			options.queryParams.page = options.currentPage;
		}
		//loading遮挡层
		options.loading = Loading.show(options.id, true);
		$.ajax({
			async: true,
			type: "get",
			dataType: "json",
			...
			error: function (msg) {
				$.ZMessage.warning("提示", "网络请求出错,请联系管理员");
			}
		});
	}
	...
	function createTableBody(obj, json) {
		...
		var options = $(obj).data('ztable').options;
		options.data = json;
		options.total = options.data.total;

		//判断是否分页，如果不分页，显示全部
		if (!options.pagination) {
			options.rows = options.data.rows.length;
		...
		$.each(options.data.rows, function (r, row) {
			if (index > 0) {
				//主键
				...
				rows_html += '<td  field="datagrid-header-check"   value="' + row[options.idField]
					+ '"  style="' + _display + '" ><input name="ck" type="checkbox"  value="'
		...
		$('#' + options.id).find('.datagrid-body').find('tr td a').each(function () {
			...
			var rowData = options.data.rows[rowIndex];
			var click = $(this).attr("onclick");
			$(this).removeAttr("onclick");
			if (click) {
				$(this).click(function (event) {
					callBlack[click](rowIndex, rowData);
		...
		options.onLoadSuccess.call(this);
	}
	...
	function showPage(obj) {
		var options = $(obj).data("ztable").options;
		//总记录数
		var html = '<div class="page datagrid-pager">';
		html += ' <span>共有' + options.total + '条记录</span>';
		//上一页
		...
		html += '<input id="datagrid_currentPage_input" name="currentPage" type="text" class="zd-input page-input" ' +
			'value="' + options.currentPage + '" >';
		html += '<span>页</span>';
		html += ' <button type="button" class="btn-blue" style="padding: 2px 5px 3px 5px;">跳转</button>';
		html += '</div>';
		//插入html
		$(obj).find('.datagrid-pager').remove();
		$(obj).append(html);
