...
$.fn.Huploadify = function(opts){
	...
	var defaults = {
		...
		fileObjName:'Filedata',//在后端接受文件的参数名称，如java中的Filedata
	...
	this.each(function(){
		...
		//先添加上file按钮和上传列表
		//var _id = $('.uploadify').length+1;
		var inputStr = '<input id="select_btn-'+_id+'" class="selectbtn" style="display:none;" type="file" name="fileselect[]"';
		inputStr += option.multi ? ' multiple' : '';
		inputStr += ' accept="';
		inputStr += getAcceptString(option.fileTypeExts);
		inputStr += '"/>';
		inputStr += '<span id="file_upload_'+_id+'-button" href="javascript:void(0)" class="huploadify-button">';
		inputStr += option.buttonText;
		inputStr += '</span>';
		var uploadFileListStr = '<div id="file_upload_'+_id+'-queue" class="uploadify-queue"></div>';
	  ...
	  fileObj = {
		  ...
		  //uploadFileList : _this.find('#file_upload_'+_id+'-queue'),
		  ...
		  funSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					var file = files[i];
					//处理模板中使用的变量
					var $html = $(option.itemTemplate.replace(/\${fileID}/g,'fileupload_'+_id+'_'+file.index).replace(/\${fileName}/g,file.name).replace(/\${fileSize}/g,formatFileSize(file.size)).replace(/\${instanceID}/g,_this.attr('id')));
					//如果是自动上传，去掉上传按钮
					if(option.auto){
					...
					$('#file_upload_'+_id+'-queue').append($html);
					//判断是否显示已上传文件大小
		  ...
		  onProgress: function(file, loaded, total) {
				var eleProgress = _this.find('#fileupload_'+_id+'_'+file.index+' .uploadify-progress');
		  ...
		  funGetProgressWidth: function(index){
		  	var eleProgressBar = _this.find('#fileupload_'+_id+'_'+index+' .uploadify-progress-bar');
		  ...
		  funGetFiles: function(e) {	
			  ...
			  //赋值相对的操作对象
			  fileObj.fileInput=$(e.target);
			  //_id _this 重新赋值选中的对象
			  _id= $(e.target).parent().attr("id");
		  ...
		  funDealFiles: function(files) {
			  ...
			  for(var i=0,len=files.length;i<len;i++){
				  ...
				  files[i].id = 'fileupload_'+_id+'_'+files[i].index;
			  }
			  //执行选择回调
		  ...
		  funDeleteFile: function(index) {
			  for (var i = 0,len=this.fileFilter.length; i<len; i++) {
					  ...
					  if (file.index == index) {
						  ...
						  _this.find('#fileupload_'+_id+'_'+index).fadeOut();
						  fileObj.fileInput.val('');
						  var fileUploadId=_this.find('#fileupload_'+_id+'_'+file.index).parent().parent().attr("id");
						  option.onCancel&&option.onCancel(file,fileUploadId);	
						  break;
					  }
			  }
			  return this;
		  ...
		  funUploadFile: function(file) {
			  ...
			  var thisfile = _this.find('#fileupload_'+_id+'_'+file.index);
				  ...
				  xhr.onreadystatechange = function(e) {
					  if (xhr.readyState == 4) {
						  ...
						  if (xhr.status == 200) {
								...
								if(fileObj.uploadOver){
									option.onUploadSuccess&&option.onUploadSuccess(originalFile, xhr.responseText,thisfile.context.id);
									//把获取的id赋值到删除按钮里面
									var _thisFile=JSON.parse(xhr.responseText)[0];
									if(_thisFile.success){
										var _thisFiles = _thisFile.attachements;
										var _thisFilesId = _thisFiles.id;
										//在指定的间隔时间后删掉进度条
										_this.find('#fileupload_'+_id+'_'+originalFile.index).find(".delfilebtn").attr("fileid",_thisFilesId);
									}
								    setTimeout(function(){
								    	_this.find('#fileupload_'+_id+'_'+originalFile.index).find(".uploadify-progress").fadeOut();
								    	_this.find('#fileupload_'+_id+'_'+originalFile.index).find(".up_percent").fadeOut();
		  ...
		  init: function() {	
			  ...
			  if (this.fileInput.length>0) {
				  //点击附件上传改变事件
				  $(_this.children().eq(0)).change(function(e) {
					  fileObj.funGetFiles(e);  
				  });
			  }
			  
			  //点击上传按钮时触发file的click事件
			  _this.find('.huploadify-button').on('click',function(e){
				//赋值相对的操作对象
				  fileObj.fileInput=$(this).parent().children().eq(0);
				  //_id _this 重新赋值选中的对象
	...
	return {
		...
		disable : function(instanceID){
			...
			parent.find('.huploadify-button').css('background-color','#888').off('click');
		},
		ennable : function(instanceID){
			//点击上传按钮时触发file的click事件
			var parent = instanceID ? $('file_upload_'+instanceID+'-button') : $('body');
		  parent.find('.huploadify-button').css('background-color','#707070').on('click',function(){
