...
My97DP.prototype = {
	...
	setRealValue: function($) {
		...
		if(_) _.value = $;
		$dp.el["realValue"] = $;
		if($dp.el.attributes&&$dp.el.attributes['validate-type'] && $dp.el.attributes['validate-type'].value=="Require"){
			if($.length!=0){
				if (hasClass($dp.el, "error")) {
					var reg = new RegExp('(\\s|^)' + "error" + '(\\s|$)');
					$dp.el.className = $dp.el.className.replace(reg, ' ');
					$dp.el.parentNode.parentNode.removeChild($dp.el.parentNode.nextElementSibling);
				}
			}
			else if($.length==0){
				if (!hasClass($dp.el, "error")) {
					$dp.el.className+=" error";
					var msg="\u8bf7\u8f93\u5165\u6b63\u786e\u7684\u76f8\u5173\u4fe1\u606f";
					$dp.el.parentNode.parentNode.innerHTML+='<span class="form-error zd-validation" pop="reqFalse"><i class="icon-merror"></i><span class="form-msg">'+msg+'</span></span>';
				}
			}
		}
	},
