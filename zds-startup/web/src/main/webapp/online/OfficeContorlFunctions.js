...
function createNewDocument(){
	...
	var offDocype = "";
	if(officeVer!=100){
		offDocype = "Word.Document";
	}
	else if(wpsVer!=100){
		offDocype = "WPS Doc";
	}
	if(offDocype!=""){
		OFFICE_CONTROL_OBJ. CreateNew(offDocype);
	}
}
...
function setActiveDocumentSaved(saved){
	OFFICE_CONTROL_OBJ.activeDocument.saved=saved;
}
...
/**
function saveFileToUrl(url,op,mode,actInsId)
{
	...
	if(IsFileOpened)
	{
		...
		"fileType="+fileType+"&fileObjects="+myData+"&op="+op+"&mode="+mode+"&actInstId="+actInsId,          //与控件一起提交的参数如："p1=a&p2=b&p3=c"
...
function closeWindow(data){
	$.portalData.setData(data,true);
}

/**
 * 显示/隐藏滚动条
 */
function hiddenOffControlScrollBar(x_hidden,y_hidden)
{
	if(OFFICE_CONTROL_OBJ){
		//水平滚动条
		OFFICE_CONTROL_OBJ.ActiveDocument.ActiveWindow.DisplayHorizontalScrollBar = !x_hidden;
		//垂直滚动条
		OFFICE_CONTROL_OBJ.ActiveDocument.ActiveWindow.DisplayVerticalScrollBar = !y_hidden;
		
	}
} 

/**
 * 域值交换--将html form的域值拷贝到Word文档的标签中
 * author xuanbaowei
 * date 2013-1-14
 */
